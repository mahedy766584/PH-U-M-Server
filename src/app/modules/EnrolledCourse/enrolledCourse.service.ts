...
const createEnrolledCourseIntoDB = async (userId: string, payload: TEnrolledCourse) => {
    ...
    if (
        totalCredits &&
        maxCredit &&
        totalCredits + currentCredit > maxCredit
    ) {
...
const updatedEnrolledCourseMarksIntoDB = async (
    facultyId: string,
    ...
    const isCurseBelongToFaculty = await EnrolledCourse.findOne({
        semesterRegistration,
        offeredCourse,
        student,
        faculty: faculty._id,
    });
    if (!isCurseBelongToFaculty) {
    ...
    if (courseMarks?.finalTerm) {
        const { classTest1, classTest2, midTerm, finalTerm } = isCurseBelongToFaculty.courseMarks;
    ...
    const result = await EnrolledCourse.findByIdAndUpdate(isCurseBelongToFaculty?._id, modifiedData, {
        new: true,
        runValidators: true,
    });
...
export const EnrolledCourseServices = {
    ...
    updatedEnrolledCourseMarksIntoDB,
